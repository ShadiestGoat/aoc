...
func makeSet(pkg string, size int) {
	dir := "./" + pkg
	_, err := os.Stat(dir)
	...
	os.WriteFile(dir + "/main.go", []byte(fmt.Sprintf(TMPL, pkg, size, size)), 0755)
}

func main() {
	for _, s := range sets {
		makeSet("set" + strconv.Itoa(s), s)
	}
