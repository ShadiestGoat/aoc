...
func (c Claw) Confirm(a, b int) bool {
	return c.PriceLoc == utils.XY{c.A[0] * a + c.B[0] * b, c.A[1] * a + c.B[1] * b}
}
...
func parseInput(inp string, add int) []*Claw {
	...
	for _, mac := range strings.Split(inp, "\n\n") {
		...
		c := &Claw{
			A:        utils.XYFromArr(utils.SplitAndParseInt(lines[0][11:], ", Y")),
			B:        utils.XYFromArr(utils.SplitAndParseInt(lines[1][11:], ", Y")),
			PriceLoc: utils.XYFromArr(utils.SplitAndParseInt(lines[2][9:], ", Y=")).Add(utils.XY{add, add}),
		}
